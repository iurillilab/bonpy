filepath = "/Users/vigji/code/bonpy/tests/assets/dataset/M1/20231201/095001"
...
def test_lazy_data_dict(asset_moviedata_folder):
    ...
    assert tuple(data_dict.keys()) == ('eye-cam_video', 
                                    'eye-cam_video_DLC', 
                                    'eye-cam_timestamps', 
                                    'random_file', 
                                    'simple_tstamp_csv', 
...
def test_csv_loading(asset_moviedata_folder):
    ...
    assert np.allclose(simple_csv_with_timestamp["time"].values[-5:], 
